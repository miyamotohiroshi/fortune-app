...
export default function AdminLookupPage() {
  const [state, action, pending] = useActionState(lookupAndSaveHistory, undefined)
        ...
        <section className="relative overflow-hidden px-6 pt-16 pb-12">
          ...
          <div className="relative z-10 mt-4">
            ...
            <p className="mt-4 text-sm text-slate-400 leading-relaxed">
              名前と生年月日を入力すると、<br />結果が履歴として自動保存されます
            </p>
          </div>
        </section>
        ...
        <section className="px-4 pb-8">
          ...
          >
            <div className="text-center mb-4">
              ...
              <p className="text-xs text-slate-500 mt-1">同じ名前・生年月日で再入力すると上書き保存されます</p>
            </div>
            ...
            <form action={action} className="space-y-4">
              ...
              <FormField icon={<ClockIcon />} label="生まれた時間" error={undefined}>
                <div className="flex items-center gap-2">
                  <input type="number" name="birthHour" min={0} max={23} placeholder="--" className={numInputClass} />
                  <span className="text-slate-400 text-sm shrink-0">時</span>
                  <input type="number" name="birthMinute" min={0} max={59} placeholder="--" className={numInputClass} />
                  <span className="text-slate-400 text-sm shrink-0">分</span>
                </div>
              </FormField>

              <FormField icon={<MapPinIcon />} label="生まれた都市" error={undefined}>
                <select name="birthCity" className={`${inputClass} appearance-none`} defaultValue="">
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  >
                    <span>{pending ? '計算中...' : '占って履歴に保存'}</span>
            ...
            <div className="mt-5 pt-4 border-t border-purple-900/20 text-center">
              <Link href="/admin/history" className="block text-xs text-slate-600 hover:text-slate-400 transition-colors">
                ← 履歴一覧に戻る
              </Link>
            </div>
          </div>
