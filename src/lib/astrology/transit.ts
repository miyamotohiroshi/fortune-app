...
export function getTransitOrb(natal: PlanetKey, transit: TransitPlanetKey): number {
...
const BODY_MAP: Record<TransitPlanetKey, Body> = {
  pluto: Body.Pluto,
...
function transitLongitude(planet: TransitPlanetKey, date: Date): number {
...
export function calculateTransitBands(
  ...
  // トランシット天体の黄経を1年分キャッシュ
  const transitLon: Record<TransitPlanetKey, number[]> = {
    pluto: [], neptune: [], uranus: [], saturn: [], jupiter: [],
  }
  for (const tp of TRANSIT_PLANETS) {
    transitLon[tp] = days.map(d => transitLongitude(tp, d))
  }
